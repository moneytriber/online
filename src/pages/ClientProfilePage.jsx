...
function ClientProfilePage() {
  ...
  const publish = async () => {
    const username = normalizeUsername(profile.publishedUsername || profile.username)
    ...
    const nextProfile = { ...profile, username, publishedUsername: username }
    const nextPublicProfile = createPublicProfile(nextProfile, riskProfile, investorKnowledge)
    setPublishState('publishing')
    setNotice('')
    try {
      await publishInvestorProfile(username, nextPublicProfile)
  ...
  const copyLink = async () => {
    ...
    try {
      ...
      setNotice('Profile published and share link copied.')
    } catch {
      setNotice('Profile published. Open the preview to copy its address.')
    }
  }
      ...
      <div className="mt-8 grid gap-6 xl:grid-cols-[0.95fr_1.05fr] xl:items-start">
        ...
        <aside className="space-y-5 xl:sticky xl:top-24">
          ...
          <div className="rounded-[24px] border border-slate-200 bg-white p-6">
            ...
            {shareUrl ? <a href={shareUrl} target="_blank" rel="noreferrer" className="mt-5 inline-flex items-center gap-2 text-sm font-bold text-[var(--mf-primary)]">Open public view <ExternalLink className="h-4 w-4" /></a> : <p className="mt-5 text-xs font-bold text-slate-500">Choose a username to create your share link.</p>}
          </div>
        </aside>
      </div>
    </div>
